  ...
  @foreach($qualities as $quality)
        ...
        >
            <label class="flex items-center justify-start cursor-pointer group border-transparent border-2 rounded-md hover:border-blue-500 w-full">
                <input type="checkbox" value="{{ $quality->id }}" wire:model="checked" class="hidden bg-transparent border-transparent rounded-full">
                <span class="p-1 w-full">{{ $quality->quality }}</span>
                <span 
                    x-show="show" 
                    style="display: none;"
                >
