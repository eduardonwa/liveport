...
>
    ...
    @endif <!-- removed qualities (bulk delete) end -->
  ...
  @foreach($qualities as $quality)
        ...
        >
            <label class="px-3 flex items-center justify-start cursor-pointer group border-transparent border-2 rounded-md hover:border-blue-500 w-full">
                <input type="checkbox" value="{{ $quality->id }}" wire:model="checked" class="focus:ring-0 bg-transparent appearance-none border-0 rounded-full">
                <span class="px-3 p-1 w-full">{{ $quality->quality }}</span>
                <span 
                    x-show="show" 
                    style="display: none;"
                >
                    <form
                        wire:submit.prevent="deleteQuality({{ $quality->id }})"
                        method="POST"
